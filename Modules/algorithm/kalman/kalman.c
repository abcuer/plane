...
X(k|k)= X(k|k-1)+Kg(k) (Z(k)-X(k|k-1))  根据预测值结合估计值得出当前状态值
P(k|k)=(1-Kg(k))P(k|k-1) 更新协方差


(k-1|k-1)上一个状态的最优估计
(k|k-1) 由上一个状态的最优估计预测当前状态的最优估计
(k|k) 由预测本状态的估计具体实现最优估计
...
注 :   P (0|0)一般不取0，取0意味在0时的方差为0，系统认为0时刻的值是最优的。然而在实际系统中往往0时刻不是最优的
 **********************************************************************************************************************/
